...
void sclPrintDeviceNamePlatforms( sclHard* hardList, int n_found )
{
	int i;
...
sclHard* sclGetAllHardware( int* n_found )
{
	int i, j; 
	...
	else
	{
		for ( i = 0; i < (int)nPlatforms; ++i )
		{
			...
			else
			{
				...
				{
					hardList[ *n_found ].platform	= platforms[ i ];
					hardList[ *n_found ].device		= devices[ j ];
					hardList[ *n_found ].nComputeUnits= _sclGetMaxComputeUnits( hardList[ *n_found ].device );
					hardList[ *n_found ].maxPointerSize= _sclGetMaxMemAllocSize( hardList[ *n_found ].device );
					hardList[ *n_found ].deviceType	= _sclGetDeviceType( hardList[ *n_found ].device );
					hardList[ *n_found ].devNum		= *n_found;
					(*n_found)++;
				}
			}
		}
...
sclHard sclGetGPUHardware( int nDevice, int* found )
{
	...
	*found = 1;
	...
	/* Get platform info */
	err = clGetPlatformIDs( 8, platforms, &nPlatforms );
	printf("Number of GPU platforms found: %d \n", nPlatforms);
...
sclHard sclGetCPUHardware( int nDevice, int* found )
{
	...
	*found = 1;
	...
	if ( nPlatforms == 0 )
	{
		printf("No OpenCL CPU plantforms found.\n");
		*found = 0;
	}
	else if ( nPlatforms == 1 )
	{
	...
	printf("Number of CPU devices found: %d \n", nDevices);
...
sclSoft sclGetCLSoftware( const char* path, const char* name, sclHard hardware )
{
	sclSoft software;
	/* Load program source */
